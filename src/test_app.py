...
class GestureAnalysis: # Same as the HadnRecog class in the Open Source Project
    ...
    def update_hand_result(self, hand_results):
        try:
        ...
        if self.finger == HandLandmarks.FIRST_TWO_OPEN:
            ...
            if ratio > 1.7:
                ...
                if self.get_dz(points[0]) < 0.1:
                    current_gesture = HandLandmarks.FIRST_TWO_CLOSE
        else:
...
class GestureController:
    ...
    def handle_controls(self, gesture, hand_result): 
        ...
        if gesture == HandLandmarks.FIRST_TWO_OPEN:
...
class App:
    ...
    def classify_hands(results):
        ...
        try: #If 1 hand in fram classifies which hand is being used
    ...
    def start(self):
        ...
        with mp_hands.Hands(
            ...
            while self.cap.isOpened():
                ...
                #image = cv2.cvtColor(cv2.flip(image, 1), cv2.COLOR_BGR2RGB) #inverted image
                ...
                #Have to change this if statement
                ...
                #     controller.handle_controls(gesture_name, hand.hand_result)
        ...
        # self.cap.release()
                if results.multi_hand_landmarks:
                    App.classify_hands(results)
                    #print("Hand: ", App.rl_hand)
                    hand.update_hand_result(App.rl_hand)
                    #print("Hand Result: ", hand.hand_result)
                    hand.set_finger_state()
                    gesture_name = hand.get_gesture() 
                ...
                cv2.imshow('Jerry the MetaMouse', cv2.flip(image, 1))
